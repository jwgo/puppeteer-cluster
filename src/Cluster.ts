    ...
    public getMonitorObject(): MonitorObject {
        ...
        const donePercentage =
            this.allTargetCount === 0
                ? 1
                : monitorObject.doneTargets / this.allTargetCount;

        monitorObject.donePercentage = (100 * donePercentage).toFixed(2);
        monitorObject.errorPerc =
            monitorObject.doneTargets === 0
                ? "0.00"
                : ((100 * this.errorCount) / monitorObject.doneTargets).toFixed(
                      2
                  );
        ...
        if (donePercentage !== 0) {
            timeRemainingMillis =
                monitorObject.timeDiff / donePercentage -
        ...
        monitorObject.memoryUsage = this.systemMonitor
            .getMemoryUsage()
            .toFixed(1);

        monitorObject.pagesPerSecond =
            monitorObject.doneTargets === 0
                ? "0"
                : (
                      (monitorObject.doneTargets * 1000) /
                      monitorObject.timeDiff
                  ).toFixed(2);
